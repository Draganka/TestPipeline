    ...
    stage('Send email with test results') {
      parallel {
        stage('Send email with test results - Selenium') {
          steps {
            emailext(subject: 'Pipeline Test Results -Selenium', body: '${FILE,path="target/surefire-reports/junitreports/html/finalreport/TESTS-TestSuites.html"}', to: 'dragana.todorchevska@interworks.com.mk')
          }
        }
